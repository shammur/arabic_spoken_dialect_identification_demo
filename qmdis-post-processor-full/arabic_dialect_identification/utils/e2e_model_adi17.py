...
class nn:
    ...
    def __init__(self, x1, y_, y_string, shapes_batch, softmax_num,is_training,input_dim, is_batchnorm):
        ...
        with tf.variable_scope("softmax") as scope:
            ...
            self.ea, self.eb, self.o1,self.res1,self.ac1,self.ac2, self.rest_1, self.rest_2, self.rest_3 = self.net(x1, shapes_batch, softmax_num,is_training,input_dim,is_batchnorm)
            
            # Create loss
            self.loss    = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y_, logits=self.o1), name="loss_mean")
        self.label=y_
    ...
    def net(self,x, shapes_batch,softmax_num,is_training, input_dim, is_batchnorm):
        ...
        # print('ConvLayer1', conv1.shape, conv1r.shape)

        ###Extra
        shape_list_tmp = tf.cast(shape_list, tf.float32)

        shape_list_tmp = tf.reshape(shape_list_tmp, [-1, 1, 1])
        # print(shape_list)
        meant = tf.reduce_sum(conv1r, 1, keep_dims=True) / shape_list_tmp
        vart = tf.sqrt(tf.reduce_sum(tf.square(conv1r - meant), 1, keep_dims=True) / shape_list_tmp)
        rest = tf.concat([meant, vart], 2)
        rest_1 = tf.squeeze(rest, axis=1)
        # print('rest1', rest_1.shape)
        ###Extra-End
        ...
        # print('ConvLayer2', conv2.shape, conv2r.shape)
        ###Extra
        shape_list_tmp = tf.cast(shape_list, tf.float32)

        shape_list_tmp = tf.reshape(shape_list_tmp, [-1, 1, 1])
        # print(shape_list)
        meant = tf.reduce_sum(conv2r, 1, keep_dims=True) / shape_list_tmp
        vart = tf.sqrt(tf.reduce_sum(tf.square(conv2r - meant), 1, keep_dims=True) / shape_list_tmp)
        rest = tf.concat([meant, vart], 2)
        rest_2 = tf.squeeze(rest, axis=1)
        # print('rest2', rest_2.shape)
        ###Extra-End
        ...
        # print('ConvLayer3', conv3.shape, conv3r.shape)
        ...
        shape_list_tmp = tf.reshape(shape_list_tmp, [-1, 1, 1])
        # print(shape_list)
        meant = tf.reduce_sum(conv3r, 1, keep_dims=True) / shape_list_tmp
        vart = tf.sqrt(tf.reduce_sum(tf.square(conv3r - meant), 1, keep_dims=True) / shape_list_tmp)
        rest = tf.concat([meant, vart], 2)
        rest_3 = tf.squeeze(rest, axis=1)
        # print('rest3', rest_3.shape)
        ###Extra-End
        ...
        # print('ConvLayer4', conv4.shape, conv4r.shape)
        # print (conv1)
        # print('ConvLayer4-pre', conv4r[0].shape, conv4r[1].shape)
        ...
        return fc1, fc2, fc3,res1,ac1,ac2, rest_1, rest_2, rest_3  #modified by shammur
